...
class Agent:
    def __init__(self, state_size, window_size, trend, skip, batch_size):
        ...
        feed = tf.layers.dense(self.X, 256, activation = tf.nn.relu) # 1 hidden layer with 256 nodes 
        ...
        self.logits = tf.layers.dense(feed, self.action_size) # output layer with 3 nodes corresponding to output node
        self.cost = tf.reduce_mean(tf.square(self.Y - self.logits)) # L2 cost function
        self.optimizer = tf.train.GradientDescentOptimizer(1e-5).minimize(
            self.cost
        ) # standard GD optimizer
